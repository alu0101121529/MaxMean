#include "../include/grasp.hpp"
 ...
 std::vector<int> Grasp::constructionPhase(std::vector<int> firstCandidates) {
   ...
   while(solution.size() < solutionSize) {
     std::vector<int> rclVector = RCL(solution, firstCandidates);
     solution.push_back(rclVector[rand() % rclVector.size()]);
   }
   return solution;
 }

 std::vector<int> Grasp::RCL(std::vector<int> solution, std::vector<int> firstCandidates) {
...
std::vector<int> Grasp::localSearch(std::vector<int> solution, std::vector<int> firstCandidates) {
...
std::pair<int,bool> Grasp::getWorstMd(std::vector<int> solution) {
